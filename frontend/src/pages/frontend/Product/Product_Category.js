...
function Product_Category() {
    ...
    useEffect(() => {
        apiProduct.getProductByCategorySlug(slug, page, limit).then((res) => {
            try {
        ...
        <div className='container'>
                    ...
                    {data.map((item, index) => (
                        ...
                        <div className="col-xl-2 col-lg-3 col-md-4 col-6" key={index} style={{ display: "flex" }}>
                            <div className="card card-sm card-product-grid pt-2" style={{ flexGrow: 1 }}>
                                ...
                                <Link to={`/san-pham/chi-tiet-san-pham/${item.slug}`} className="img-wrap"> <img src={imageURL + item.image} style={{ maxWidth: "100%", objectFit: "contain" }} /> </Link>
            ...
            <ul className="pagination">
                <li className="page-item">
                    {page > 1 ? (
                        <Link className="page-link" to={`/danh-muc/${slug}/${page - 1}/${limit}`}>Previous</Link>
                    ) : (
                ...
                {Array.from(Array(pages).keys()).map((index) => (
                    ...
                    >
                        <Link
                            ...
                            to={`/danh-muc/${slug}/${index + 1}/${limit}`}
                        >
                            {index + 1}
                        </Link>
                    </li>
                ...
                <li className="page-item">
                    {page < pages ? (
                        <Link className="page-link" to={`/danh-muc/${slug}/${page + 1}/${limit}`}>
                            Next
                        </Link>
                    ) : (
