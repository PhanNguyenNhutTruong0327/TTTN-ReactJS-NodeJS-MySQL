...
const getAll = async (db, page, limit) => {
    return new Promise((resolve, reject) => {
        db.query(`SELECT 
                (SELECT COUNT(*) FROM post WHERE status = 0 AND type = 'page') AS qty_trash,
                (SELECT COUNT(*) FROM post WHERE status != 0 AND type = 'page') AS qty`,
            (err, qty) => {
                if (err) {
                    reject(err);
                    return;
                ...
                db.query(
                    `SELECT p.*
                    FROM post p 
                    WHERE p.status != 0 AND type = 'page'
...
const getPageBySlug = async (db, slug) => {
    return new Promise((resolve, reject) => {
        db.query(`SELECT * FROM post WHERE status = 1 AND type = 'page' AND slug = ?`, [slug],
            (err, res) => {
                if (err) {
                    reject(err);
                    return;
